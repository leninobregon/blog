...
if (isset($_POST['action']) && $_POST['action'] === 'install') {
    ...
    try {
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS `comments` (`id` INT AUTO_INCREMENT PRIMARY KEY, `post_id` INT NOT NULL, `user_id` INT NOT NULL, `content` TEXT NOT NULL, `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (`post_id`) REFERENCES `posts`(`id`) ON DELETE CASCADE, FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        
        $pdo->exec("CREATE TABLE IF NOT EXISTS `newsletter` (`id` INT AUTO_INCREMENT PRIMARY KEY, `email` VARCHAR(100) NOT NULL UNIQUE, `name` VARCHAR(100), `active` TINYINT(1) DEFAULT 1, `token` VARCHAR(100), `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        
        $pdo->exec("CREATE TABLE IF NOT EXISTS `visit_logs` (`id` INT AUTO_INCREMENT PRIMARY KEY, `page` VARCHAR(255), `ip` VARCHAR(45), `user_agent` TEXT, `referer` VARCHAR(255), `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS `site_stats` (`id` INT AUTO_INCREMENT PRIMARY KEY, `total_hits` INT DEFAULT 0, `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
